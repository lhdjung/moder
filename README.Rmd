...
The moder package determines single or multiple modes (most frequent values). By default, its functions check whether missing values make this impossible, and return `NA` in this case. Its source code has no dependencies.
...
But what if some values are missing? Maybe there are so many missings that it's impossible to tell which value is the most frequent one. If all `NA`s in the next example are secretly `2`, this value is the (first) mode. Otherwise, `1` is. The mode is unclear, so the function returns `NA`:
