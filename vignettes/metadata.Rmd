...
All this only works if the full set of modes can be determined. Below, `NA` could secretly be `7`, `8`, or any other value. If it's `8`, both numbers are equally frequent. Otherwise, `7` is the only mode. Since we lack this information, the number of modes is unknown.
...
Use `mode_count_range()` in such cases. It will determine the minimal and maximal number of modes, never returning `NA`:
...
With `mode_frequency_range()`, at least the minimal and maximal frequencies can be determined. It never returns `NA`. There are four instances of `1` without counting the `NA`s, and six with counting it:
