...
The mode functions return `1` for the vector below, while `mean()` and `median()` return `NA`. I still think it makes sense to compare them because this difference may occur in many realistic distributions: Some values are missing, so the mean and the median are unknown, but there are enough known values to determine at least one mode.
