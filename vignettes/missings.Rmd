...
In `mode_single()`, `accept` is `TRUE` by default. This avoids returning `NA` when some, but not all modes are known. Set it to `FALSE` if it's important to draw the single mode from the complete set of modes, not from the minimum that just so happens to be known. If the complete set can't be determined, the function returns `NA`:

```{r}
x5 <- c(4, 4, 4, 7, 7, NA)
mode_single(x5)
mode_single(x5, accept = FALSE)
```

Why is this different from `mode_first()` with its `accept = FALSE` default? The latter function specifically asks for the first mode, so the default should be to get that or `NA`. However, `mode_single()` just wants one mode, so it's fine to accept any one by default even if not all modes are known.
